...
class TimePostingDao_mySqlTest {
  ...
  static void setUp() {
    final String fileLocation = Objects.requireNonNull(TimePostingDao_mySqlTest.class
        .getClassLoader()
        .getResource("time_post/mysql_time_post_sql.yaml"))
        .getPath();
  ...
  void createWorklog() {
    ...
    final PendingTime pendingTime = pendingTimeHelper.getPendingTime().orElseThrow(() ->
  ...
  private Optional<PendingTime> getPendingTime() {
    return fluentJdbc.query().select("select * from wt_post_time")
        .firstResult(rs -> new PendingTime()
            .setCaseId(rs.getString("case_id"))
  ...
  public static class FlywayTestDbModule extends AbstractModule {
    ...
    private static class FlywayProviderTestImpl implements Provider<Flyway> {
      ...
      public Flyway get() {
        ...
        flyway.setLocations("db_schema/mysql/");
        return flyway;
      }
    }
  }
